 ...
 */
@Service
public class StockService4_2 implements StockService {
    ...
    public List<OneRes> run(List<StockDetail> list){
        ...
        for(int i = 1; i <= list.size()-getDayNum();i++) {
            ...
            if(before.getPricePert().compareTo(BigDecimal.ZERO) <0) {
                continue;
            }
    ...
    public String getStrategy() {
        return String.format("%s    %s",this.getClass().getSimpleName(), "涨幅成交比 比前一天大 且缩量 当天和前一天都是红") ;
    }
