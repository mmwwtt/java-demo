...
@Service
public class StockService15 implements StockService {
    ...
    public List<OneRes> run(List<StockDetail> list) {
        ...
        for (int i = 1; i <= list.size() - getDayNum(); i++) {
            ...
            //上影线长度占实体1/3以上  表示抛压大
            if (stockDetail.getUpperShadowLength() / stockDetail.getAllLength() > 0.40) {
                continue;
            }
            //上影线长度占实体1/3以上  表示抛压大
            if (stockDetail.getUpperShadowLength() / stockDetail.getAllLength() < 0.20) {
                continue;
            }
