    ...
    RISE5_MAX_MIDDLE("rise5MaxMiddle", "最大五日涨幅中位数",
            ...
            (StrategyTmp tmp) -> {
                ...
                if (lessThan(tmp.getPert(), multiply(tmp.getParentPert(), 1.01))
                        ...
                        || (level == 3 && (lessThan(tmp.getPert(), 0.077) || tmp.getDateCnt() < 60 || tmp.getRise5MinMiddle() < -0.055))
                        || (level == 4 && (lessThan(tmp.getPert(), 0.087) || tmp.getDateCnt() < 55 || tmp.getRise5MinMiddle() < -0.05))
                        || (level == 5 && (lessThan(tmp.getPert(), 0.098) || tmp.getDateCnt() < 55 || tmp.getRise5MinMiddle() < -0.05))
                        || (level == 6 && (lessThan(tmp.getPert(), 0.105) || tmp.getDateCnt() < 55 || tmp.getRise5MinMiddle() < -0.05))
                        || (level == 7 && (lessThan(tmp.getPert(), 0.115) || tmp.getDateCnt() < 50 || tmp.getRise5MinMiddle() < -0.045))
